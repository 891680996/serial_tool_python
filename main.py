...
# parity
SERIAL_CHECKBIT_ARRAY = (serial.PARITY_EVEN,serial.PARITY_NONE, 
...
class MplCanvas(FigureCanvas):
    def __init__(self):
        ...
        self.fig.subplots_adjust(left=0.08, right=0.99, top=0.9, bottom=0.1)
        ...
        self.plotdatabuf = []
        # 指定图形的字体  
        # self.fig.fontsize.yticks(fontsize=50)
        #self.ax.yaxis.set_ticks_possition('right')
        #self.fig.set_xticks(fontsize='20')
...
class CKZS(Ui_optical_communication.Ui_Form):
    ...
    def start_button_Clicked(self):
        ...
        if clickstatus:  # 如果开关为打开状态
            ...
            databit = 8
            # print(self.set_data_bit_box.currentIndex())
            # print(type(self.set_data_bit_box.currentIndex()))
            # print(databit)
            stopbit = 1
            # print(self.set_stop_bit_box.currentIndex())
            # print(stopbit)
            ...
            if self._serial.isOpen():
                self.timer.start(0.001)    # 定时器30ms刷新一次界面
    ...
    def tr(self):
        self.threads = []
        t1 = threading.Thread(target=self.shou)
        self.threads.append(t1)
        t2 = threading.Thread(target=self.fa)
